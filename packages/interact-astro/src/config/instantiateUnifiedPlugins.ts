...
function determinePluginType(pluginConfigPath: string, pluginConfig: PluginConfig | null): string {
    ...
    if (pluginConfig != null && typeof pluginConfig.type != 'undefined') {
    ...
    throw new Error(`The type of the plugin ${pluginConfig.path} is not set and the path does not contain ${remark} or ${rehype}`)
