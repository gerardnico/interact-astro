...
async function generateImage(masterFilePath: string, dryRun: boolean, outputDirectory: string) {
    if (!masterFilePath) {
        masterFilePath = config.theme.site.masterFavicon
    }
    this.log(`Generating Favicons and Manifest with the master file: ${masterFilePath}`)
...
export default class GenerateFavicon extends Command {
    ...
    async run(): Promise<void> {
        ...
        await generateImage(filePath, dryRun, outputDirectory)
    }
}